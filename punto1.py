...
def verificar_complejidad(resultados, I_xy, umbral_C=0.6, umbral_I=0.1):
    print("\n" + "=" * 65)
    print("  VERIFICACIÓN FORMAL DE COMPLEJIDAD")
    print("=" * 65)

    # Condición 1: entropía en escenario normal > umbral
    res_normal = next(r for r in resultados if r['escenario'] == 'normal')
    C_normal = res_normal['complejidad']
    cond1 = C_normal > umbral_C
    print(f"\n  [C1] Entropía normalizada en operación normal: {C_normal:.2%}")
    print(f"       Umbral: C > {umbral_C:.0%}  →  {'CUMPLE' if cond1 else 'NO CUMPLE'}")

    # Condición 2: información mutua > 0 (interdependencia)
    cond2 = I_xy > umbral_I
    print(f"\n  [C2] Información mutua entre nodos: I(X;Y) = {I_xy:.4f} bits")
    print(f"       Umbral: I > {umbral_I}  →  {'CUMPLE' if cond2 else 'NO CUMPLE'}")

    # Condición 3: H_max crece con n (escalabilidad)
    n_internet = 5_000_000_000
    H_internet = np.log2(n_internet)
    cond3 = H_internet > 30  # arbitrariamente alto
    print(f"\n  [C3] H_max para ~5×10⁹ dispositivos: {H_internet:.2f} bits")
    print(f"       →  {'CUMPLE' if cond3 else 'NO CUMPLE'} (complejidad estructural ilimitada)")

    todas = cond1 and cond2 and cond3
    print(f"\n  VEREDICTO: Internet {'ES' if todas else 'NO ES'} un sistema complejo")
    print(f"  bajo el modelo de Shannon (las 3 condiciones {'se cumplen' if todas else 'no se cumplen'}).")
...
def main():
   ...
   #Visualizaciones
    ...
    resultados = [analizar_escenario(e, N_NODOS) for e in ['uniforme', 'normal', 'congestion', 'ddos']]
    verificar_complejidad(resultados, I_xy)
