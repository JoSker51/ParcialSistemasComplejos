...
def regla_erase(epsilon: Agente, alpha: Agente, red: Red):
    """ε ⊳◁ α → coloca ε en cada puerto auxiliar libre de α"""
    desconectar(epsilon.principal, alpha.principal)
    for i in range(1, alpha.aridad + 1):
        if not alpha.aux(i).libre():
            vecino = alpha.aux(i).wire
            desconectar(alpha.aux(i), vecino)
            nuevo_eps = red.agregar(Agente("ε", 0))
            conectar(nuevo_eps.principal, vecino)
    red.remover(epsilon)
    red.remover(alpha)



def multiplicar(x: int, y: int, verbose=True) -> int:
    """
    mult(0, y)    = 0          → m ⊳◁ 0: resultado es 0, borrar y con ε
    mult(S(x), y) = add(mult(x,y), y)  → m ⊳◁ S: recursión + duplicar y con δ
    Se implementa iterativamente: mult(x,y) = y + y + ... + y (x veces)
    """
    global _id_counter
    _id_counter = 0
    ...
    resultado = 0
    for _ in range(x):
        resultado = sumar(resultado, y, verbose=False)
